<template>
    <div v-if="permsArray.includes('variations_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="variations">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('variation.add')"
            :successMessage="$t('variation.created')"
        />
    </div>
</template>
<script>
