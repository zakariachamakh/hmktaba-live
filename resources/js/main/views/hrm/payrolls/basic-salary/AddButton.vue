<template>
    <div
        v-if="
            permsArray.includes('basic_salaries_create') || permsArray.includes('admin')
        "
    >
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="basic_salaries">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('basic_salary.add')"
            :successMessage="$t('basic_salary.created')"
        />
    </div>
</template>
<script>
